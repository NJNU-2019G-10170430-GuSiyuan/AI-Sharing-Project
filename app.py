...
with main_container:
    ...
    # ====== 第二页：互动游戏 ======
        ...
        if 'shuffled_scenarios' not in st.session_state:
            raw_scenarios = {
                ...
                "给你一个新业务线的目标，你自己去网上查行业研报、调用分析插件，最后给我生成一份市场调研 PPT？（注：路径未知）": "智能体 (Agent)",
    ...
    # ====== 第三页：工作流 vs 智能体 ======
        ...
        if mode == "👷‍♂️ 指派【工作流】按 SOP 处理":
            ...
            st.success(
                "🏁 **最终决策 (Finish):** 暂缓一切机器自动回复。已生成【紧急待办工单】并强提醒涉事店长，要求 5 分钟内电话致歉。")
    ...
    # ====== 第五页：高阶实战（消灭幻觉） ======
        ...
        if st.session_state.get("btn_chat_math"):
            with st.spinner("AI 正在疯狂捏造数据..."):
                time.sleep(1)
                st.error(
                    "❌ **[纯大模型] 的回答：**\n\n根据我对行业的了解，浙江大区上周的客单价大概在 1500-2000 元左右，整体呈现上升趋势。杭州门店表现最好，大概增长了 15%。\n\n*(锐评：你哪来的数据？AI 出现了严重的数据幻觉！)*")
        ...
        if st.session_state.get("btn_agent_math"):
            with st.spinner("Agent 正在自主生成 SQL -> 查询内网数据库 -> 渲染图表..."):
                ...
                st.success(
                    "✅ **[数据库 Agent] 的结论：**\n\n已成功连接 BI 数据库。如上方折线图所示：杭州门店上周三客单价有一次异常峰值（1850元），经查关联数据，是因为当天下单了多台成色极好的高值机。整体大区转化率环比稳定持平。")
    ...
    # ====== 第六页：原理解密（搜索的本质） ======
        ...
        if st.button("🚀 对比搜索过程", use_container_width=True):
            ...
            with col_db:
                ...
                st.error(
                    "❌ **搜索失败：报表返回 0 条结果**\n\n**崩溃原因：** 数据库就像个极其死板的审计员，它的表里明明存着一条叫《iPhone 外观严重破损定级规范》的规则，但因为没有“苹果”、“碎成渣”这些精确字眼，它什么也找不到。")
        ...
        st.markdown("""
        💡**速记金句：**
        > **想要查“某个店长上个月准确的退单率”，必须接数据库（Agentic）；**
    ...
    # ====== 第七页：数据库的高阶隐藏用法 ======
        ...
        > **这叫“动态组装，精准投喂”，极大节省了 API 成本，且准确率高达 99%。**
        """)
    # ====== 第八页：插件机制（给 AI 装上双手） ======
        ...
        with col_demo:
            ...
            if st.session_state.get("plugin_running"):
                ...
                with st.status("🤖 AI 正在调用插件操作表格...", expanded=True) as status:
                    ...
                    st.session_state.mock_feishu_base.at[0, "AI 处理备注"] = "命中敏感词[投诉]，已通过飞书抄送店长"
    ...
    # ====== 第十页：大结局与留资收口 ======
        ...
        st.write("不管是客服客诉、合规稽查，还是数据自动面板，只要有 SOP，我们就能把它变成 AI 工作流。")

        col_qr, col_text = st.columns([1, 2])

        with col_qr:
            # 放置二维码（加了 try-except 防止你还没放图片时代码报错停机）
            try:
                st.image("photo.png", width=200, caption="扫码提交 AI 提效需求")
            except Exception as e:
                st.warning("⚠️ 请将名为 photo.png 的二维码图片放到代码同目录下")

        with col_text:
            st.info("""
            👇 **扫左侧二维码，填报你的业务痛点：**
            1. 描述你对本次分享哪个环节的落地最感兴趣。
            2. 给本次的分享创意一个评价。

            *“AI 不会淘汰企业，但掌握了 AI 工作流的企业，会淘汰剩下的。”*
            """)
            st.balloons()  # 大结局撒花
